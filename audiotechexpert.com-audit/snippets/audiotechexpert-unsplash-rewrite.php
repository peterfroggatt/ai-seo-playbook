 ...
 * Description: Rewrites hotlinked images.unsplash.com/photo-* URLs to your self-hosted copies — in rendered content (Elementor HTML widgets, inline CSS, <img>) AND in Yoast's og:image / Twitter image / schema ImageObject. Only rewrites photos you've actually uploaded; others are left untouched. Guarded against double-load so a duplicate file can never fatal the site.
 * Version:     1.1.1
 * Author:      SEO audit remediation
 *
 * INSTALL: wp-content/mu-plugins/ (auto-activates). Keep exactly ONE copy (no "(1)" duplicate).
 * Then LiteSpeed Cache > Toolbox > Purge All. Uploaded files must be named after the Unsplash
 * photo id (e.g. photo-1484704849700-f032a568e944.jpg). Remove the file to revert.
 */
...
// Guard: if this file (or a duplicate copy) is loaded more than once, load the logic only once.
if ( ! defined( 'ATE_UNSPLASH_REWRITE_LOADED' ) ) {
	define( 'ATE_UNSPLASH_REWRITE_LOADED', true );
	...
	/** Rewrite all occurrences inside a blob of HTML. */
	function ate_unsplash_rewrite( $html ) {
		return ate_unsplash_map_url( $html );
	}

	/** Recursively rewrite image URLs inside Yoast's schema graph array. */
	...
	// 2) Yoast Open Graph + Twitter images.
	add_filter( 'wpseo_opengraph_image', 'ate_unsplash_map_url', 20 );
	add_filter( 'wpseo_twitter_image', 'ate_unsplash_map_url', 20 );
	// 3) Yoast schema graph ImageObject.
	add_filter( 'wpseo_schema_graph', 'ate_unsplash_rewrite_graph', 999 );
